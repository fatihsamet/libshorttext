...
class Analyzer:
	...
	def analyze_single(self, target, amount = 5, output = None, extra_svm_feats = []):
		...
		total = 0
		for a in weights_table[-1]: 
			if float(a) > 0:
				total = total+float(a)
		percents =[]
		for a in weights_table[-1]:
			if float(a)>0:
				percents.append(float(a)/total)
			else:
				percents.append(0)
		featset = dict()
		for idx,feat in enumerate(features):
			fdict = dict(zip(labels,[float(i) for i in weights_table[idx] ]))
			featset[feat] = fdict
		dictionary = dict(zip(labels,[float(i) for i in weights_table[-1]]))
		percentdict = dict(zip(labels,percents))
		return (dictionary,percentdict,featset)
